...
def getxycenter( imfile, x, y, ext=0, radec=False,
    ...
    """
    from PythonPhot.cntrd import cntrd
    import pyfits
...
FLUX_VEGA_ACS_WFC_INF = {'F435W':6.384e-09,'F475W':5.290e-09,'F502N':4.660e-09,
                         'F550M':3.416e-09,'F555W':3.811e-09,'F606W':2.869e-09,
                         'F625W':2.351e-09,'F658N':1.776e-09,'F660N':1.932e-09,
                         'F775W':1.287e-09,'F814W':1.134e-09,'F850LP':8.261e-10,
                         'F892N':8.729e-10 }
...
def getzptACS( image, system='Vega', ext=0 ):
    ...
    if system.lower()=='vega':  zpt = zptdat['VEGAMAG'][ifilt]
    elif system.lower()=='st':  zpt = zptdat['STMAG'][ifilt]
    elif system.lower()=='ab':  zpt = zptdat['ABMAG'][ifilt]
...
def dophot( imfile, xc, yc, aparcsec=0.4, system='AB', ext=None,
            ...
            ntestpositions=100, testboxsize=10,
            snthresh=0.0, zeropoint=None, skyannarcsec=[6.0,12.0],
            skyval=None, skyalgorithm='sigmaclipping', target=None,
            printstyle=None, exact=False, fitsconvention=True, phpadu=None,
            returnflux=False, verbose=False, debug=False ):
    ...
    Inputs:
      ...
      zeropoint : fix the zeropoint (if not provided, we look it up from hardcoded tables)
    ...
    """
    from dophotometry import get_flux_and_err
    import pyfits
    import numpy as np
    import hstapcorr
    ...
    if 'FILTER1' in hdr0 :
        if 'CLEAR' in hdr0['FILTER1'] : filtname = hdr0['FILTER2']
        else : filtname = hdr0['FILTER1']
    else :
    ...
    assert skyannpix[0] >= np.max(appix), "Sky annulus must be >= largest aperture."
    camera = getcamera(hdr0)
    ...
    if phpadu is None :
        if 'BUNIT' not in hdr0 :
            ...
            phpadu = 1
        assert (phpadu is not None), "Can't determine units from the image header."
    ...
    for i in range(len(apflux)) :
        ...
        apflux[i] = apflux[i] * 10**(0.4*apcor[i]) # flux rescaled to larger aperture
        df = apfluxerr[i] * 10**(0.4*apcor[i])   # flux error rescaled
        dfap = 0.4*np.log(10)*apflux[i]*aperr[i] # systematic err from aperture correction
        ...
        if apflux[i]<abs(apfluxerr[i])*snthresh :
            ...
            mag[i] = -2.5 * np.log10( sigmafactor *abs(apfluxerr[i]) ) + zpt - apcor[i]
            magerr[i] = -9.0
        else :
            # Good detection. convert to a magnitude (ap correction already applied)
            mag[i] = -2.5 * np.log10(apflux[i]) + zpt
            magerr[i] = 1.0857 * apfluxerr[i] / apflux[i]
        if verbose > 2 :
            print( " %.4f  %.4f  %.4f  %.4f"%(apflux[i], apflux[i] / 10**(0.4*apcor[i]),df,dfap))
    ...
    if 'EXPEND' in hdr :
        ...
        mjdobs = 0.0
    if not target : target = imfile.split('_')[0]

    if verbose and printstyle=='snana' :
        # Convert to SNANA fluxcal units and Construct a SNANA-style OBS line, e.g.
        # OBS: 56456.500  H  wol    0.000    8.630   25.160   -9.000
        fluxcal = apflux * 10**(0.4*(27.5-zpt))
        fluxcalerr = apfluxerr * 10**(0.4*(27.5-zpt))
        print( "VARLIST:  MJD  FLT FIELD   FLUXCAL   FLUXCALERR    MAG     MAGERR   ZPT" )
    elif verbose :
        if printstyle.lower() in ['long','verbose']:
            print("#  TARGET                RA         DEC       MJD  FILTER  APER       FLUX  FLUXERR         MAG   MAGERR  MAGSYS       ZP      SKY SKYERR  IMAGE")
        else :
            print("# MJD     FILTER  APER      FLUX   FLUXERR       MAG     MAGERR  MAGSYS    ZP       SKY   SKYERR")

    maglinelist = []
    for iap in range(len(aparcsec)) :
        if printstyle=='snana' :
            magline = "OBS: %8.2f   %6s   %s %8.3f %8.3f    %8.3f %8.3f   %.3f"%(
                float(mjdobs), FilterAlpha[filtname], target,
                fluxcal[iap], fluxcalerr[iap], mag[iap], magerr[iap], zpt  )
            ...
            magline = '%-15s  %10.5f  %10.5f  %.2f  %6s  %4.2f  %9.4f %8.4f   %9.4f %8.4f  %5s   %7.4f  %7.4f %6.4f  %s'%(
                target,  xout, yout,  float(mjdobs), filtname, aparcsec[iap],
                apflux[iap], apfluxerr[iap], mag[iap], magerr[iap], system, zpt, sky, skyerr, imfile )

        else :
            magline = '%.2f  %6s  %4.2f  %9.4f %8.4f   %9.4f %8.4f  %5s   %7.4f  %7.4f %6.4f'%(
                float(mjdobs), filtname, aparcsec[iap],
                apflux[iap], apfluxerr[iap], mag[iap], magerr[iap], system, zpt, sky, skyerr )
...
def main():
    ...
    parser.add_argument('--psfmodel',type=str, default=None, help="Filename of a psf model fits file.")
    parser.add_argument('--ext', type=int,default=None,help='Specify the fits extension number. Required for FLT files.')
    parser.add_argument('--forced',action='store_true',help='Forced photometry mode (no recentering).')
    parser.add_argument('--radec',action='store_true',help='x,y give RA and Dec instead of pixel position.')
    parser.add_argument('--AB',action='store_true',help='Report AB mags (the default).')
    parser.add_argument('--vega',action='store_true',help='Report Vega mags.')
    parser.add_argument('--exact',action='store_true',help='Compute sub-pixel areas exactly (Slower. Necessary for small apertures.)')
    parser.add_argument('--apertures',type=str,default='0.4',help='Size of photometry aperture(s) in arcsec. ')
    parser.add_argument('--skyannulus',metavar='6.0,12.0',type=str,default='6.0,12.0',help='Inner and outer radius of sky annulus in arcsec. ')
    parser.add_argument('--skyval',type=float,default=None,help='Fix the sky flux per pixel to this value.')
    parser.add_argument('--skyalgorithm',choices=['sigmaclipping','mmm'], default='sigmaclipping', help='Set the algorithm to use for measuring the sky.')
    parser.add_argument('--zeropoint',type=float,default=None,help='Fix the zero point. If not provided, we use a hardcoded table for HST filters.')
    parser.add_argument('--snthresh',type=float,default=0,help='When the S/N is below this threshold, the magnitude is printed as an N-sigma lower limit.')
    parser.add_argument('--printstyle',type=str, choices=['snana','long','short'], default='short', help='Report photometry in long format or as a SNANA-style "OBS:" line.')
    parser.add_argument('--target',type=str, default=None, help="Name of target (for 'long' and 'snana' print styles)")
    parser.add_argument('--phpadu',type=float,default=None,help='Photons per ADU (for converting data numbers to photon counts).')
    parser.add_argument('-v',dest='verbose',action='count', help='Turn verbosity up (use -v,-vv,-vvv, etc.)')
    parser.add_argument('-d',dest='debug',action='count',help='Turn up debugging depth (use -d,-dd,-ddd)')
    ...
    if ( argv.image.endswith('flt.fits') or argv.image.endswith('flc.fits') ) :
        if argv.ext is None :
            raise exceptions.RuntimeError('For FLT files you must specify the fits extension number.')
    if ('NEXTEND' in hdr ) :
        if hdr['NEXTEND'] > 1 and argv.ext is None :
            raise exceptions.RuntimeError('For MEF files you must specify the fits extension number.')

    if argv.radec :
        xim,yim = radec2xy( argv.image, argv.x, argv.y, ext=argv.ext)
        if argv.verbose > 1 :
            print( "x,y position [fits-style, (1,1)-indexed]: %.2f %.2f"%(xim,yim) )
    else :
    ...
    magsys = 'AB'
    if argv.vega : magsys = 'Vega'
    if argv.AB : magsys = 'AB'
    ...
    maglinelist = dophot( argv.image, xim, yim, aplist,
                          ...
                          skyalgorithm=argv.skyalgorithm, snthresh=argv.snthresh,
                          exact=argv.exact,
                          recenter=False, # recentering already done above
                          printstyle=argv.printstyle, target=argv.target,
                          phpadu=argv.phpadu, verbose=argv.verbose, debug=argv.debug )
