...
PACKAGENAME = metadata.get('package_name', 'hstphot')
DESCRIPTION = metadata.get('description', 'Simple aperture and psf-fitting photometry for isolated sources in HST images.')
AUTHOR = metadata.get('author', 'srodney')
AUTHOR_EMAIL = metadata.get('author_email', '')
LICENSE = metadata.get('license', 'unknown')
URL = metadata.get('url', 'https://github.com/srodney/hstphot')
__minimum_python_version__ = metadata.get("minimum_python_version", "2.7")

# Enforce Python version check - this is the same check as in __init__.py but
# this one has to happen before importing ah_bootstrap.
if sys.version_info < tuple((int(val) for val in __minimum_python_version__.split('.'))):
    sys.stderr.write("ERROR: hstphot requires Python {} or later\n".format(__minimum_python_version__))
    sys.exit(1)
